        ...
        <el-menu mode="horizontal">
          ...
          <el-menu-item>
            ...
            <div class="border-content">
              ...
              <el-switch
                      v-model="switchStatus2"
                      @change="switchChange2"
                      active-text="0~120小时"
                      active-color="#03B452"
                      inactive-color="#7E7772"></el-switch>
              <el-checkbox-group v-model="fhour2" @change="changeFhour2">
                <el-checkbox
                        v-for="item in ftimeView2"
                        :label="item"
                        :key="item">{{ item == "0" ? "综合" : item + "时" }}</el-checkbox>
              </el-checkbox-group>
            </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        fhour2: [0, 24, 48, 72, 96, 120],
        ftimeView2: [0, 24, 48, 72, 96, 120],
    ...
    methods: {
      ...
      initEcharts() {
        ...
        let options = {
          ...
          xAxis: {
            // categories: ['综合(0~72)', '24(0~72)', '48(0~72)', '72(0~72)', '综合(0~120)','24(0~120)', '48(0~120)', '72(0~120)', '96(0~120)', '120(0~120)'],
          ...
          //   {
          //     name: '中央台',
          //     data: [76.5, 78.62, 72.15,76.5, 78.62, 72.15,76.5, 78.62, 72.15,76.58]
          //   },
          //   {
          //     name: '省台指导',
          //     data: [71.5, 77.62, 73.15,76.5, 78.62, 72.15,76.5, 78.62, 72.15,73.56]
          //   },
          //   {
          //     name: '地市订正',
          //     data: [86.5, 68.62, 70.15,76.5, 78.62, 72.15,76.5, 78.62, 72.15,75.39]
          //   },
          //   {
          //     name: 'GRAPES_GFS',
          //     data: [66.5, 88.62, 76.15,76.5, 78.62, 72.15,76.5, 78.62, 72.15,74.12]
          //   }
          // ],
        };
      ...
      initEchData() {
        let {fhour, fhour2, models, modelOptions, facname, data} = this
        let categories = []
        for (let i = 0; i < fhour.length; i++) {
          if (fhour[i] === 0) {
            categories.push('综合(0~72)')
          } else {
            categories.push(fhour[i] + '(0~72)')
          }
        }
        for (let i = 0; i < fhour2.length; i++) {
          if (fhour2[i] === 0) {
            categories.push('综合(0~120)')
          } else {
            categories.push(fhour2[i] + '(0~120)')
          }
        }
        let series = []
        for (let i = 0; i < models.length; i++) {
          ...
          for (let j = 0; j < categories.length; j++) {
            ...
            for (let k = 0; k < data.length; k++) {
              let cat
              if (data[k]['wfhour'] === 0) {
                cat = '综合(0~' + data[k]['wfhours'] + ')'
              } else {
                cat = data[k]['wfhour'] + '(0~' + data[k]['wfhours'] + ')'
              }
      ...
      initTable() {
        let {models, modelOptions, fhour, fhour2, data, facname} = this
        let tableHeader = []
        let tableData = []
        ...
        for (let i = 0; i < fhour.length; i++) {
          let row = {}
          fhour[i] === 0 ? row['ftime'] = '综合(0~72)' : row['ftime'] = fhour[i] + '(0~72)'
          for (let j = 0; j < models.length; j++) {
            let colData = NaN
            for (let k = 0; k < data.length; k++) {
              if (fhour[i] === data[k]['wfhour'] && data[k]['wfhours'] === 72 && data[k]['wfsrc'] === models[j]) {
        ...
        for (let i = 0; i < fhour2.length; i++) {
          let row = {}
          fhour2[i] === 0 ? row['ftime'] = '综合(0~120)' : row['ftime'] = fhour2[i] + '(0~120)'
          for (let j = 0; j < models.length; j++) {
            let colData = NaN
            for (let k = 0; k < data.length; k++) {
              if (fhour2[i] === data[k]['wfhour'] && data[k]['wfhours'] === 120 && data[k]['wfsrc'] === models[j]) {
                colData = data[k][facname]
              }
            }
            row[models[j]] = isNaN(colData) ? '/' : colData.toFixed(2)
